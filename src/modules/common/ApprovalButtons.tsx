...
export default function ApprovalButtons({ className, status, handleUndo, handleApprove, handleDecline, isLoading, targetStatus, canApprove, ...rest }: IProps) {
    return (
        <div {...rest}
            ...
            {(status === 'APPROVED' || status === 'DECLINED' || status === 'approved' || status === 'rejected' || status === 'cancelled' || status === 'completed') && handleUndo && < Button onClick={handleUndo} variant="outline" disabled={isLoading}>{isLoading && (targetStatus === 'PENDING' || targetStatus === 'pending') && <Spinner size="sm" />} UNDO</Button>}
            {(status === 'PENDING' || status === 'pending' || status === 'on_going') && handleApprove && < Button onClick={handleApprove} variant="outline" disabled={isLoading || !canApprove}>{isLoading && (targetStatus === 'APPROVED' || targetStatus === 'approved') && <Spinner size="sm" />} Approve</Button>}
