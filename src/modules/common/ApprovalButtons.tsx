...
export default function ApprovalButtons({ className, status, handleUndo, handleApprove, handleDecline, isLoading, targetStatus, canApprove, ...rest }: IProps) {
    return (
        <div {...rest}
            ...
            {(status === 'PENDING' || status === 'pending' || status === 'applied' || (!handleUndo && status === 'declined') || status === 'on_going') && handleApprove && < Button onClick={handleApprove} variant="outline" disabled={isLoading || !canApprove}>{isLoading && (targetStatus === 'APPROVED' || targetStatus === 'approved') && <Spinner size="sm" />} Approve</Button>}
            {(status === 'PENDING' || status === 'pending' || (!handleUndo && status === 'approved') || status === 'on_going') && handleDecline && < Button onClick={handleDecline} variant="outline" disabled={isLoading}>{isLoading && (targetStatus === 'DECLINED' || targetStatus === 'rejected') && <Spinner size="sm" />} Decline</Button>}
        </div>
    );
}
